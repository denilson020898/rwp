...
mod jwt_tests {
    ...
    fn decode_from_request_with_correct_token() {
        let encoded_token: String = JwtToken::encode(32);
        let request = test::TestRequest::with_header("user-token", encoded_token)
            .to_http_request();
    ...
    fn decode_from_request_with_no_token() {
        let request = test::TestRequest::with_header("test", "test")
            .to_http_request();
        let out_come = JwtToken::decode_from_request(request);
    ...
    fn decode_from_request_with_false_token() {
        let request = test::TestRequest::with_header("user-token", "test")
            .to_http_request();
        let out_come = JwtToken::decode_from_request(request);
